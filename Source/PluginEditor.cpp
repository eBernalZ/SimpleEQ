...
void LookAndFeel::drawRotarySlider(juce::Graphics& g, int x, int y, int width, int height, float sliderPosProportional, float rotaryStartAngle, float rotaryEndAngle, juce::Slider& slider) {
    ...
    g.setColour(enabled ? Colour(97u,18u,167u) : Colours::darkgrey); // COLOR MORADO DE LOS ELIPSES
    g.fillEllipse(bounds);
    
    g.setColour(enabled ? Colour(255u,154u,1u) : Colours::grey); // COLOR NARANJA DE LAS ORILLAS
...
void LookAndFeel::drawToggleButton(juce::Graphics &g, juce::ToggleButton &toggleButton, bool shouldDrawButtonAsHighlighted, bool shouldDrawButtonAsDown) {
    ...
    else if (auto* analyzerButton = dynamic_cast<struct AnalyzerButton*>(&toggleButton)) {
        auto color = ! toggleButton.getToggleState() ? Colours::dimgrey : Colour(0u, 172u, 1u);
        g.setColour(color);
...
//    g.drawRect(sliderBounds);
    ...
    g.setColour(Colour(0u, 172u, 1u)); // COLOR VERDE
...
void SimpleEQAudioProcessorEditor::resized()
{
    ...
    float hRatio = 28.f / 100.f; // JUCE_LIVE_CONSTANT(25) / 100.f;
